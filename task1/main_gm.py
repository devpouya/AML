...
sel = SelectKBest(f_regression, k =200)
...
################model######################
X_train, X_train_val, y_train, y_train_val = train_test_split(train_normalized, label_del, test_size = 0.01, random_state = 0, shuffle = True)


clf =svm.SVR(C=25.0, cache_size=200, coef0=0.0, degree=3, epsilon=0.1,
...
tuned_parameters = [{'C':[10,20,25,30,45,50]}]

grid = GridSearchCV(clf ,  param_grid=tuned_parameters,  scoring=r2_scorer , cv=10)
...
train_split_pred = clf.predict(X_train)
score_on_train = r2_score(train_split_pred, y_train)
print("score on train split: {}".format(score_on_train))
...
sample.to_csv("y_pred_svr_f_reg_c.csv", index = False)